...
class Joiner():
    ...
    def __init__(self, Ligand_Set, Receptor_Set):
        ...
        """
        assert type(Ligand_Set) is type(Receptor_Set) and len(Ligand_Set) == len(Receptor_Set)
        assert False not in [type(Ligand_Set[i]) is type(Receptor_Set[i]) for i in range(3)]
