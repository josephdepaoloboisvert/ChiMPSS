...
class LigandPreparation():
    ...
    def _prepare_ligand_BC(self):
        ...
        if self.data['BC'].protocol == []:
            ...
            minimizedB_FN = None
            if self.args.FNs['score'] is not None:
            ...
            if minimizedB_FN is not None and os.path.isfile(minimizedB_FN):
                ...
                # Only save configurations if we have any and a valid filename
                if minimizedB_FN is not None and len(minimizedConfigurations) > 0:
                    from netCDF4 import Dataset
            ...
            if MMTK:
                ...
                seeds = [np.copy(self.top.configuration().array)
                         for n in range(self.args.params['BC']['seeds_per_state'])]
        else:
            seeds = None
        return seeds
    ...
    def _prepare_ligand_CD(self):
        ...
        if self.data['CD'].protocol == []:
            # Use alpha=1.0 to keep full grid interactions during ligand preparation
            # This keeps the ligand properly oriented in the binding site
            ...
            if seeds == []:
                ...
                if MMTK:
                    ...
                    seeds, 'CD', self.log, self.data)
                    if len(seeds) > 0:
                        self.top.setConfiguration(np.copy(seeds[-1]))

                # Save trajectory before temperature ramp for debugging
                if os.environ.get('ALGDOCK_SAVE_TRAJECTORIES', 'false').lower() == 'true' and len(seeds) > 0:
                    self._save_ligand_prep_trajectory(seeds, 'CD_ligprep_before_temp_ramp.pdb')
                ...
                # Get current configuration after temperature ramp
                if MMTK:
                    final_conf = np.copy(self.top.universe.configuration().array)
                else:
                    final_conf = np.copy(self.top.configuration().array)

                # Save trajectory after temperature ramp for debugging
                if os.environ.get('ALGDOCK_SAVE_TRAJECTORIES', 'false').lower() == 'true':
                    self._save_ligand_prep_trajectory([final_conf], 'CD_ligprep_after_temp_ramp.pdb')

                # Create seeds by duplicating the equilibrated configuration
                # This matches MMTK behavior - all seeds start identical
                # Diversity comes from different random velocities during sampling
                if MMTK:
                    seeds = [np.copy(self.top.universe.configuration().array)
        ...
        for T in T_SERIES:
            ...
            while attempts_left > 0:
                ...
                if self.args.random_seed > 0:
                    ...
                    random_seed = (self.args.random_seed + int(T*10000) + attempts_left) % 32767
                else:
                    # Position-based seed for production runs
                    if MMTK:
            ...
            if MMTK:
                delta_t_fs = delta_t * 1000  # MMTK Units
            else:
                delta_t_fs = delta_t.value_in_unit(unit.femtosecond)  # OpenMM Quantity
            self.log.tee(fmt % (T, delta_t_fs, steps_per_trial, acc_rate))
    ...
    def _save_ligand_prep_trajectory(self, confs, filename):
        """Save ligand preparation configurations as multi-model PDB for debugging
        ...
        try:
            ...
            self.log.tee(f"  Saved {len(confs)} structures to {filename}")
        except Exception as e:
            self.log.tee(f"  Warning: Could not save trajectory to {filename}: {e}")