...
from chimpss.algdock import path_tools
from chimpss.algdock.BindingPMF import DEBUG
...
try:
    ...
    MMTK = None
    Configuration = None
    # MMTK internal unit system: lengths in nm, energies in kJ/mol.
    # 1 Ang = 0.1 nm, 1 kcal/mol = 4.184 kJ/mol, kJ and mol factors = 1.0.
    class _Units:
        Ang = 0.1      # 1 Å in nm (MMTK internal length unit)
        kcal = 4.184   # 1 kcal/mol in kJ/mol (MMTK internal energy unit)
        kJ = 1.0       # 1 kJ/mol = 1.0 (MMTK base energy unit)
        mol = 1.0      # dimensionless in MMTK molar unit system
    if MMTK is None:
        class _MMTK:
            Units = _Units()
        MMTK = _MMTK()
...
class Postprocessing:
  ...
  top_RL : chimpss.algdock.topology.TopologyMMTK
  ...
  def run(self,
    ...
    # Write trajectories and collect tasks
    tasks = []
    time_per_snap = {}  # Threads share memory, no need for Manager
    for (p, state, c, moiety, phase) in incomplete:
      if moiety + phase not in time_per_snap.keys():
        time_per_snap[moiety + phase] = []
    ...
    for (p, state, c, moiety, phase) in incomplete:
      ...
      # IMPORTANT: Copy confs to avoid race conditions when multiple threads
      # process the same configurations with different phases
      import copy
      tasks.append((copy.deepcopy(confs), moiety, phase, traj_FN, outputname, debug, \
              (p,state,c,moiety+phase), time_per_snap))
    ...
    with ThreadPoolExecutor(max_workers=self.args.cores) as executor:
      results = list(executor.map(lambda task: self._energy_task(*task), tasks))
  ...
  def _energy_task(self, confs, moiety, phase, traj_FN, outputname, debug, reference, time_per_snap):
    """Performs energy calculation for one task (thread-based)
    """
    (p, state, c, label) = reference
    nsnaps = len(confs)

    # Make sure there is enough time remaining
    if len(time_per_snap.get(moiety + phase, [])) > 0:
      if not self.log.isTimeForTask(nsnaps * np.array(time_per_snap[moiety + phase])):
        return None

    # Calculate the energy
    args = (confs, moiety, phase, traj_FN, outputname, debug, reference)
    ...
    if not np.isinf(E).any():
      ...
      # Store timing (threads share memory, can modify dict directly)
      if moiety + phase not in time_per_snap:
        time_per_snap[moiety + phase] = []
      time_per_snap[moiety + phase].append(wall_time / nsnaps)

      # Return result (instead of putting in queue)
      return (E, reference, wall_time)
    else:
  ...
  def _NAMD_Energy(self,
    ...
    # Run NAMD
    import chimpss.algdock.NAMD
    energyCalc = chimpss.algdock.NAMD.NAMD(\
  ...
  def _OpenMM_Energy(self, confs, moiety, phase, traj_FN=None, \
    ...
    E = []
    for conf in confs:
      # Use lock to ensure thread-safe access to OpenMM context
      with self._OpenMM_locks[key]:
        self._OpenMM_sims[key].context.setPositions(conf)
        s = self._OpenMM_sims[key].context.getState(getEnergy=True)
        # Return format: [0., total_energy] to match old behavior
        E.append([0., s.getPotentialEnergy() / openmm.unit.kilojoule * openmm.unit.mole])
  ...
  def _setup_OpenMM(self, moiety, phase):
    if not hasattr(self, '_OpenMM_sims'):
      self._OpenMM_sims = {}
      self._OpenMM_locks = {}
      self._OpenMM_setup_lock = threading.Lock()

    key = moiety + phase
    # Use a lock to prevent race conditions during setup
    with self._OpenMM_setup_lock:
      if not key in self._OpenMM_sims.keys():
        ...
        if moiety == 'R':
          ...
          # For MMTK: self.top.universe.numberOfAtoms()
          # For OpenMM: self.top.numberOfAtoms()
          num_ligand_atoms = self.top.numberOfAtoms() if hasattr(self.top, 'numberOfAtoms') else self.top.universe.numberOfAtoms()
          receptor_indices = list(range(self.top_RL.L_first_atom)) + \
              list(range(self.top_RL.L_first_atom + num_ligand_atoms, OMM_system.getNumParticles()))
          for i in receptor_indices:
            OMM_system.setParticleMass(i, 0)
        # Set force groups so we can query energies by component
        for i in range(OMM_system.getNumForces()):
          OMM_system.getForce(i).setForceGroup(i)

        dummy_integrator = openmm.LangevinIntegrator(300*openmm.unit.kelvin, \
          1/openmm.unit.picosecond, 0.002*openmm.unit.picoseconds)
        self._OpenMM_sims[key] = OpenMM_app.Simulation(prmtop.topology, \
          OMM_system, dummy_integrator)
        # Create a lock for thread-safe access to this context
        self._OpenMM_locks[key] = threading.Lock()
...
'''.format(bcfl, dime, gcent, grid, sdie))
  ...
  def _write_traj(self, traj_FN, confs, moiety, \
    ...
    if traj_FN.endswith('.dcd'):
      IO_dcd = chimpss.algdock.IO.dcd(self.top.molecule,
